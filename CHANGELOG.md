...
#### Schema Registry — чтение файлов сервера и запросы по любым протоколам через `URLРеестра`
Адрес реестра передавался в libcurl (`CURLOPT_URL`) без ограничения протоколов, а проверка `isValidUrl()` во всех пяти методах Schema Registry (`ЗарегистрироватьСхему`, `ПолучитьСхемуПоИД`, `ПолучитьПоследнююСхему`, `ПолучитьВерсииСхемы`, `УдалитьСхему`) была закомментирована. libcurl принимал любую схему, собранную в библиотеке: `ПолучитьПоследнююСхему("file:///etc/passwd#", "x")` возвращал содержимое файла с сервера 1С (дописываемый компонентой путь `/subjects/x/versions/latest` уходил во фрагмент URL), а `gopher://`, `dict://`, `ftp://`, `scp://` позволяли обращаться от имени сервера к внутренним сервисам (SSRF). Сама `isValidUrl()` при этом не работала: регулярное выражение было записано многострочной raw-строкой с отступами, а `std::regex::extended` (POSIX ERE) не знает `\d` и `\s` — на libc++ конструктор бросал `std::regex_error` для любого непустого адреса.

Теперь `httpRequest()` разрешает libcurl только HTTP и HTTPS — и для запроса, и для перенаправлений (`CURLOPT_PROTOCOLS_STR` / `CURLOPT_REDIR_PROTOCOLS_STR`, при сборке с libcurl старше 7.85.0 — `CURLOPT_PROTOCOLS` / `CURLOPT_REDIR_PROTOCOLS`). Если ограничение установить не удалось, запрос не выполняется. `isValidUrl()` переписана на разбор адреса парсером самого libcurl (`curl_url()`): требуются явная схема `http` или `https` (регистр не важен) и хост, не допускаются query-строка, фрагмент, пробелы и управляющие символы. Учётные данные в адресе (`https://ключ:секрет@реестр`) допускаются: другого способа передать реестру Basic-аутентификацию в компоненте нет. Проверка снова включена во всех пяти методах; при ошибке метод возвращает пустую строку (`УдалитьСхему` — `Ложь`), текст `Некорректный URL Schema Registry: ...` доступен через `ПолучитьСообщениеОбОшибке`.

Адрес без схемы (`localhost:8081`) раньше срабатывал, потому что libcurl сам подставлял `http://`; теперь схему нужно указывать явно.

#### Schema Registry — `Субъект` и `Версия` подставлялись в путь URL без кодирования
Значения вставлялись в URL как есть, поэтому `/`, `?`, `#` и сегменты `..` меняли адресуемый метод API реестра. Например, `УдалитьСхему(URLРеестра, "users-value", "..")` после нормализации пути в libcurl отправляло `DELETE /subjects/users-value/` — удаление всего субъекта вместо одной версии, `"../../config"` превращалось в `DELETE /subjects/config`, а `"1?permanent=true"` добавляло к запросу параметр безвозвратного удаления. Теперь `Субъект` и `Версия` кодируются через `curl_easy_escape()` (всё, кроме `A-Z a-z 0-9 - . _ ~`, заменяется на `%XX`), а пустые значения и `.` / `..`, которые кодирование не меняет, отвергаются с ошибкой `Некорректное имя субъекта Schema Registry: ...` / `Некорректная версия схемы: ...`.
...
[Unreleased]: https://github.com/NuclearAPK/Simple-Kafka_Adapter/compare/v1.9.5...HEAD
